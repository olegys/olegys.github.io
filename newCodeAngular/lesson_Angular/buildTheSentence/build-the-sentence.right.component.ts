...
@Component( {
    ...
    template: `
        ...
        <div class="addNewItems" [ngClass]="{'hidden': rows['question_data'].length === restrictionRows}">
            <button md-raised-button class="addButton select green" (click)="createNewRow(2)">
                <md-icon>add</md-icon>
...
export class BuildTheSentenceRightComponent {
     ...
     */
    private restrictionInput: number = RESTRICTIONS[ RESTRICTIONS[ 'programType' ] ][ 'build-the-sentence' ].input;
    private restrictionRows: number = RESTRICTIONS[ RESTRICTIONS[ 'programType' ] ][ 'build-the-sentence' ].rows;
